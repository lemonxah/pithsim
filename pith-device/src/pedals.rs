...
impl Pedals {
    ...
    /// DDU/handbrake): `@OTA<size>` -> OTAREADY, then raw bytes ACK'd per 2048
    /// with "K", OTADONE + device reboot on success. The device stops its
    /// telemetry/status stream during the transfer; any stale line is skipped.
    pub fn ota_upload(
        &mut self,
        img: &[u8],
